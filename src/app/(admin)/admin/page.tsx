...
import { DollarSign, Package, Users, Activity, ArrowUpRight, Loader2 } from "lucide-react";
    ...
    return (
        <div className="p-6 md:p-10 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard title="Total Revenue" value={`$${stats?.revenue?.toLocaleString() || '0.00'}`} change="+20.1%" icon={DollarSign} color="bg-green-500" />
                <StatCard title="Active Shipments" value={stats?.activeShipments || 0} change="+180 new" icon={Package} color="bg-brand-blue" />
                <StatCard title="Total Customers" value={stats?.totalCustomers || 0} change="+12% mo" icon={Users} color="bg-[#ff1493]" />
                <StatCard title="Pending Requests" value={stats?.pendingRequests || 0} change="Requires Action" icon={Activity} color="bg-orange-500" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Main Content (Quick Actions & Reports - taking left 2/3) */}
                <div className="lg:col-span-2 space-y-8">
                    {/* Quick Actions */}
                    <div className="bg-white rounded-[32px] p-8 shadow-xl shadow-brand-blue/5 border border-white/50">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-xl font-bold text-slate-800">Quick Actions</h3>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <button className="h-32 rounded-3xl bg-brand-blue/5 border border-brand-blue/10 flex flex-col items-center justify-center gap-3 hover:bg-brand-blue/10 transition-colors group">
                                ...
                                <span className="font-bold text-brand-blue">Add Shipment</span>
                            </button>
                            <button className="h-32 rounded-3xl bg-orange-50 border border-orange-100 flex flex-col items-center justify-center gap-3 hover:bg-orange-100 transition-colors group">
                                ...
                                <span className="font-bold text-orange-600">Resolve Issues</span>
                            </button>
                        </div>
                    </div>
                </div>

                {/* Sidebar (Audit Logs - taking right 1/3) */}
                <div className="space-y-6">
                    ...
                    <div className="bg-white/60 backdrop-blur-xl border border-white/60 rounded-[32px] p-6 shadow-xl shadow-brand-blue/5 h-[500px] overflow-y-auto custom-scrollbar">
...
function ActivityItem({ title, desc, time, icon: Icon }: any) {
    return (
        <div className="flex items-center gap-4 p-4 hover:bg-slate-50 rounded-2xl transition-colors cursor-pointer group">
            <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-brand-blue group-hover:text-white transition-colors">
                <Icon size={18} />
            </div>
